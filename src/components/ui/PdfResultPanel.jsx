...
export default function PdfResultPanel({
  ...
  return (
    ...
    >
      ...
      <div className="mt-4 flex items-center gap-3 rounded-xl border border-slate-200/70 bg-slate-50/70 p-3 dark:border-slate-700/60 dark:bg-slate-800/50">
        <span className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-lg border border-primary-100 bg-primary-50 text-primary-600 dark:border-primary-500/20 dark:bg-primary-500/10 dark:text-primary-400">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-0.5 flex flex-wrap items-center gap-x-3 gap-y-0.5 text-xs text-slate-500 dark:text-slate-400">
            {pageCount && <span className="tabular-nums">{pageCount} page{pageCount === 1 ? '' : 's'}</span>}
            <span className="font-semibold tabular-nums text-slate-700 dark:text-slate-300">
              {formatBytes(blob.size)}
            </span>
            {originalSize && <span className="tabular-nums">from {formatBytes(originalSize)}</span>}
          </div>
        </div>
      </div>
